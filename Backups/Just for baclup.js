...
function updateResectionVisibility() {
  if (!distalResectionPlane || !boneMeshes['Right Femur']) return;

  const femur = boneMeshes['Right Femur'];
  ...
  if (isResectionVisible) {
    femur.material.clippingPlanes = [clippingPlane];
  } else {
    femur.material.clippingPlanes = [];
  }

  femur.material.needsUpdate = true;
...
function createMechanicalAxisPlane() {
  ...
  // Remove all previous projected lines
  ...
  // Project TEA onto the plane and store the returned line
  ...
  // Enable plane visibility buttons
...
function updateToggleButtonStyle() {
  ...
  if (isResectionVisible) {
    ...
    toggleButton.textContent = 'Resection On';
  } else {
    toggleButton.classList.remove('active');
    toggleButton.textContent = 'Resection Off';
  }
}
    ...
    increment.addEventListener('click', () => {
      switch(index) {
        ...
        case 1:
          ...
          break;
        case 2:
          distalMedialResection = Math.min(distalMedialResection + 1, 20);
          updateDistalResectionPlane();
          break;
      }
      updateControlValues();
